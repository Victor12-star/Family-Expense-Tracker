...
export default function Join() {
  ...
  async function joinFamily(e) {
    ...
    try {
      ...
      setIsError(true);
      setMessage(err.response?.data?.message || "Could not join the family.");
    }
  }
  ...
  return (
    <main className="auth-screen">
      <form className="auth-card" onSubmit={joinFamily}>
        <h1>Join a family</h1>
        <p className="tag">Enter the invitation code you received from a family owner.</p>

        {message && (
          <div className={isError ? "error-banner" : "success-banner"} role="status">
            {message}
          </div>
        )}
